<!-- Banner superior (opcional) -->
<p align="center">
  <img src="https://i.imgur.com/ZLTCG8k.gif" width="900"/>
</p>

<h1 align="center">Esteban Mendez</h1>
<h3 align="center">Python Developer • QA Automation • Data Analyst • AI Validation</h3>

<p align="center">
  <a href="https://linkedin.com/in/mendez-esteban">
    <img src="https://img.shields.io/badge/LinkedIn-Esteban%20Mendez-blue?style=for-the-badge&logo=linkedin">
  </a>
  <a href="mailto:mendez92.job@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact%20Me-red?style=for-the-badge&logo=gmail">
  </a>
  <a href="https://portf.maitweb.com">
    <img src="https://img.shields.io/badge/Portfolio-Online-black?style=for-the-badge&logo=firefox">
  </a>
</p>

---

## ⚡ What I Do

- 🔍 **QA Automation & AI Validation**  
  Testing GPT-based agents (text, images & reasoning), story-based test design, acceptance criteria, and E2E validation across backend, frontend, and databases.

- 📊 **Data Analysis & KPIs**  
  Exploratory data analysis, SQL/ETL pipelines, KPI definition (p95 latency, uptime, detections), and operational insights.

- 📈 **Observability & Dashboards**  
  Grafana dashboards based on PostgreSQL & CloudWatch metrics. Log standardization and monitoring logic for AI services.

- 🤖 **ML Projects (CV & NLP)**  
  CNN & YOLO models, NLP sentiment analysis, dataset pipelines, and evaluation metrics (accuracy, recall, mAP).

- 🧪 **Testing Frameworks**  
  Test scripting with Python & PyTest, documentation maintenance, Jira workflows, QA automation logic.

- 🧑‍💻 **Web Engineering**  
  Websites with HTML, CSS, JS, WordPress. UX/UI improvements and client technical support.

---

## 🎯 Objective

Grow as a **Data & QA Automation Engineer**, specializing in **AI systems, observability, reproducible testing pipelines, and automated validation**.

---

# 🧠 Skills Matrix

### **Languages**

![Python](https://img.shields.io/badge/-Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Bash](https://img.shields.io/badge/-Bash-121011?style=for-the-badge&logo=gnu-bash&logoColor=white)
![SQL](https://img.shields.io/badge/-SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)

---

### **Frameworks & Libraries**

![PyTest](https://img.shields.io/badge/-PyTest-0A9EDC?style=for-the-badge&logo=pytest)
![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=for-the-badge&logo=pytorch)
![sklearn](https://img.shields.io/badge/-Scikit--Learn-F7931E?style=for-the-badge&logo=scikitlearn)
![Pandas](https://img.shields.io/badge/-Pandas-150458?style=for-the-badge&logo=pandas)
![NumPy](https://img.shields.io/badge/-NumPy-013243?style=for-the-badge&logo=numpy)

---

### **Observability**

![Grafana](https://img.shields.io/badge/-Grafana-F46800?style=for-the-badge&logo=grafana)
![CloudWatch](https://img.shields.io/badge/-AWS%20CloudWatch-FF4F8B?style=for-the-badge&logo=amazon-aws)

---

### **Databases**

![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-316192?style=for-the-badge&logo=postgresql)
![SQLite](https://img.shields.io/badge/-SQLite-07405E?style=for-the-badge&logo=sqlite)

---

### **Dev Tools**

![Git](https://img.shields.io/badge/-Git-F05033?style=for-the-badge&logo=git)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=for-the-badge&logo=docker)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

---

# 🚀 Projects

### **🔹 Reddit Sentiment Extraction (NLP)**

- Sentiment analysis and classification using NLP models
- Automated conversation analysis in Python

### **🔹 Image Classification & Object Detection (CV)**

- CNN & YOLO training using PyTorch & TensorFlow
- Dataset pipelines, augmentations & metrics (accuracy, recall, mAP)

### **🔹 Cybersecurity RAM Recovery Test**

- RAM forensics using Volatility 3
- Malware, process and network anomaly detection

---

# 📊 GitHub Stats

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=scheffan&show_icons=true&theme=radical&count_private=true" />
  <img height="170" src="https://github-readme-streak-stats.herokuapp.com?user=scheffan&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=scheffan&layout=compact&theme=radical" height="170">
</p>

---

# 🐍 Contribution Snake

<p align="center">
  <img src="https://github.com/scheffan/scheffan/blob/output/github-contribution-grid-snake-dark.svg">
</p>

---

## 🔗 Contact

- 📧 **Email:** mendez92.job@gmail.com
- 🌐 **Portfolio:** https://portf.maitweb.com
- 💼 **LinkedIn:** https://linkedin.com/in/mendez-esteban

---

<h3 align="center">🚀 Always learning. Always building.</h3>
